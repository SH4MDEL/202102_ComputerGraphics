...
struct ObjectManager {
	...
	void initTexture(GLuint s_program) {
		glGenTextures(1, &field.texture); //--- 텍스처 생성

		char Text[100];
		int widthImage, heightImage, numberOfChannel;
		stbi_set_flip_vertically_on_load(true);
		glBindTexture(GL_TEXTURE_2D, field.texture); //--- 텍스처 바인딩
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); //--- 현재 바인딩된 텍스처의 파라미터 설정하기
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		sprintf(Text, "mol_ru\\field.jpg");
		field.image = stbi_load(Text, &widthImage, &heightImage, &numberOfChannel, 0); //--- 텍스처로 사용할 비트맵 이미지 로드하기
		glTexImage2D(GL_TEXTURE_2D, 0, 3, widthImage, heightImage, 0, GL_RGB, GL_UNSIGNED_BYTE, field.image); //---텍스처 이미지 정의

		stbi_image_free(field.image);

		field.initTexture(s_program);
...
GLvoid drawScene()
{
	...
	glUniform3f(viewPos, 0.0f, 0.0f, 3.0f);
	...
	cameraPos = glm::vec3(3.0f, 1.5f, 3.0f); //--- 카메라 위치
	...
	viewPos = glGetUniformLocation(s_program, "viewPos"); //--- 바라보는 방향 프래그먼트 셰이더로 전달

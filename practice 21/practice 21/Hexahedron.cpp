...
std::uniform_int_distribution<int> color(1, 10);
std::uniform_int_distribution<int> size(5, 10);
...
Hexahedron::Hexahedron()
{
	...
	GLfloat front_left_down_x = -linear, front_left_down_y = -linear, front_left_down_z = linear;		// 직육면체를 만들고 싶을 때 적절히 수정해줍니다.
	GLfloat front_left_top_x = -linear, front_left_top_y = linear, front_left_top_z = linear;		
	GLfloat front_right_down_x = linear, front_right_down_y = -linear, front_right_down_z = linear;
	GLfloat front_right_top_x = linear, front_right_top_y = linear, front_right_top_z = linear;		
	GLfloat back_left_down_x = -linear, back_left_down_y = -linear, back_left_down_z = -linear;		
	...
	GLfloat back_color_r = color(gen) / 10.0f, back_color_g = color(gen) / 10.0f, back_color_b = color(gen) / 10.0f;
...
void Hexahedron::allReset()
{
	status = 2;
	scale = 1.0f;
	xPos = coordinate(gen) / 100.0f;
	zPos = coordinate(gen) / 100.0f;
}
...
void Hexahedron::putFactor(glm::mat4 inputFactor)
{
	...
	myFactor = glm::scale(myFactor, glm::vec3(scale, scale, scale));
}
...
void Hexahedron::ccheck(GLfloat mx, GLfloat mz)
{
	if ((xPos - HeightFromGround < mx + 0.2f && xPos + HeightFromGround > mx - 0.2f) && (zPos - HeightFromGround < mz + 0.1f && zPos + HeightFromGround > mz - 0.1f)) {
		status = 1;
		return;
	}
	return;

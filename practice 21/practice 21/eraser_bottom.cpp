...
void eraser_bottom::allReset()
{
	xPos = 0.0f, zPos = 0.0f;
	HeightFromGround = 0.1f;
}

void eraser_bottom::update()
{

}

void eraser_bottom::putFactor(glm::mat4 inputFactor)
{
	myFactor = inputFactor;
	myFactor = glm::translate(myFactor, glm::vec3(xPos, HeightFromGround, zPos));	// 기본으로 세팅할 객체의 위치
}

glm::mat4 eraser_bottom::getFactor()
{
	return myFactor;
}

void eraser_bottom::w()
{
	zPos += 0.05f;
}

void eraser_bottom::s()
{
	zPos -= 0.05f;
}

void eraser_bottom::a()
{
	xPos += 0.05f;
}

void eraser_bottom::d()
{
	xPos -= 0.05f;
}
